layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("/admin/article/list", function(data){
		var newArray = [];
		data=data.payload;
		//单击首页“待审核文章”加载的信息
	...
	$("body").on("click",".news_edit",function(){  //编辑
        ...
        var index = layui.layer.open({
            title : "编辑文章",
            type : 2,
            content : "/admin/article/"+_this.attr("data-id"),
            success : function(layero, index){
                layui.layer.tips('点击此处返回文章列表', '.layui-layer-setwin .layui-layer-close', {
                    tips: 3
                });
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td>'+currData[i].hits+'</td>';
			    	if(currData[i].status == "publish"){
                        dataHtml += '<td>'+currData[i].status+'</td>';
                    }else{
                        dataHtml += '<td style="color:#f00">'+currData[i].status+'</td>';
                    }
                    var check=currData[i].allowComment? "checked":"";
			    	dataHtml += '<td>'+currData[i].categories+'</td>'
			    	+'<td><input type="checkbox" name="show" lay-skin="switch" lay-text="是|否" lay-filter="isShow"'+check+'></td>'
			    	+'<td>'+timestampToTime(currData[i].created)+'</td>'
			    	+'<td>'
					+'<a class="layui-btn layui-btn-mini news_edit" data-id="'+data[i].cid+'"><i class="iconfont icon-edit"></i> 编辑</a>'
					+'<a class="layui-btn layui-btn-danger layui-btn-mini news_del" data-id="'+data[i].cid+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
				}
			}else{
